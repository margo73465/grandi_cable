...
int main() {
  ...
  FILE *outputCV;
  FILE *inputIC;
  outputI2 = fopen("grandiI2_cable_3c6_1Hz_def_10AP.dat","w");
  outputI = fopen("grandiI1_cable_3c6_1Hz_def_10AP.dat","w");

  //Delete contents of data files:
  outputVdmax = fopen("grandi_Vdmax_cable_3c6_1Hz_def_10AP.dat","w");
  fclose(outputVdmax);
  outputAPD = fopen("grandi_APD_cable_3c6_1Hz_def_10AP.dat","w");
  fclose(outputVdmax);
  outputCV = fopen("grandi_CV_cable_3c6_1Hz_def_10AP.dat","w");
  fclose(outputVdmax);
  ...
  while (t < t_end) {
	...
	for (cc=0; cc<5; cc++) {
	  for (z=1; z<DIM-1; z++)  {
		Vn[z] = dt_max/(dx*dx*5)*D*(Vo[z+1]+Vo[z-1]-2.0*Vo[z]) + Vo[z];
	  }
	...
	if (Vn[49]<=-60. && VP[49]>-60.) {
	  outputAPD = fopen("grandi_APDnot90_cable_3c6_1Hz_def_10AP.dat","a");
	...
	if (Vn[49]<=0. && VP[49]>0.) {
	  outputVdmax = fopen("grandi_Vdmax_cable_3c6_1Hz_def_10AP.dat","a");
	...
	if (Vn[49] <= V_90 && VP[49] > V_90 && t-APDstart>0){
	  outputAPD = fopen("grandi_APD90_cable_3c6_1Hz_def_10AP.dat","a");
	  fprintf(outputAPD, "%.2f \t %.2f \n",t,t-APDstart);
	  fclose(outputAPD);
	  }*/
	...
	if (Vn[79]>=-40. && VP[79]<-40.) {
	  outputCV = fopen("grandi_CV_cable_3c6_1Hz_def_10AP.dat","a");
