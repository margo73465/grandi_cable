...
/* Cable parameters */
const double D = 0.001; //cm^2/ms
const double dx = 0.025; //cm
const int DIM = 100;
const int rec1 = 3;
const int rec2 = 49;
...
int main() {
  ...
  //FILE *outputVdmax;
  FILE *inputIC;
  outputI2 = fopen("grandiI2_cable_3c5_1Hz_10AP.dat","w");
  outputI = fopen("grandiI1_cable_3c5_1Hz_10AP.dat","w");
  ...
  double dRYRr[DIM], dRYRo[DIM], dRYRi[DIM], dbuf_Naj[DIM], dbuf_Nasl[DIM]; 
  double dbuf_TnC_low[DIM], dbuf_TnC_high[DIM], dbuf_Mg[DIM], dbuf_CaM[DIM], dbuf_myoCa[DIM], dbuf_myoMg[DIM], dbuf_SR[DIM]; 
  double dbuf_SLLj[DIM], dbuf_SLLsl[DIM], dbuf_SLHj[DIM], dbuf_SLHsl[DIM];
  double dNaj[DIM], dNasl[DIM], dNai[DIM], dCaj[DIM], dCasl[DIM], dCai[DIM], Ki[DIM];
  ...
  while (t < t_end) {
    ...
    for (z=1; z<DIM-1; z++) {  
	  ...
	  dRYRr[z] = (kim*RI-temp2*Caj[z]*RYRr[z])-(temp*Caj[z]*Caj[z]*RYRr[z]-kom*RYRo[z]);   // R
	  dRYRo[z] = (temp*Caj[z]*Caj[z]*RYRr[z]-kom*RYRo[z])-(temp2*Caj[z]*RYRo[z]-kim*RYRi[z]);// O
	  dRYRi[z] = (temp2*Caj[z]*RYRo[z]-kim*RYRi[z])-(kom*RYRi[z]-temp*Caj[z]*Caj[z]*RI);   // I
	  ...
	  dbuf_Naj[z] = 0.1e-3*Naj[z]*(7.561-buf_Naj[z])-1.0e-3*buf_Naj[z];        // NaBj      [mM/ms]
	  dbuf_Nasl[z] = 0.1e-3*Nasl[z]*(1.65-buf_Nasl[z])-1.0e-3*buf_Nasl[z];       // NaBsl     [mM/ms]
	  
	  // Cytosolic Ca Buffers
	  dbuf_TnC_low[z] = 32.7*Cai[z]*(70.0e-3-buf_TnC_low[z])-(1.+0.5*ISO)*19.6e-3*buf_TnC_low[z];            // TnCL      [mM/ms]
	  dbuf_TnC_high[z] = 2.37*Cai[z]*(140.0e-3-buf_TnC_high[z]-buf_Mg[z])-0.032e-3*buf_TnC_high[z]; // TnCHc     [mM/ms]
	  dbuf_Mg[z] = 3.0e-3*(140.0e-3-buf_TnC_high[z]-buf_Mg[z])-3.33e-3*buf_Mg[z];   // TnCHm     [mM/ms]
	  dbuf_CaM[z] = 34.*Cai[z]*(24.0e-3-buf_CaM[z])-238.0e-3*buf_CaM[z];                 // CaM       [mM/ms]
	  dbuf_myoCa[z] = 13.8*Cai[z]*(140.0e-3-buf_myoCa[z]-buf_myoMg[z])-0.46e-3*buf_myoCa[z];    // Myosin_ca [mM/ms]
	  dbuf_myoMg[z] = 0.0157*(140.0e-3-buf_myoCa[z]-buf_myoMg[z])-0.057e-3*buf_myoMg[z];      // Myosin_mg [mM/ms]
	  dbuf_SR[z] = 100.*Cai[z]*(19.*.9e-3-buf_SR[z])-60.0e-3*buf_SR[z];                    // SRB       [mM/ms]
	  J_CaB_cytosol = dbuf_TnC_low[z]+dbuf_TnC_high[z]+dbuf_Mg[z]+dbuf_CaM[z]+dbuf_myoCa[z]+dbuf_myoMg[z]+dbuf_SR[z];
	  
	  // Junctional and SL Ca Buffers
	  dbuf_SLLj[z] = 100.*Caj[z]*(4.6e-4*Vmyo/Vjunc-buf_SLLj[z])-1300.0e-3*buf_SLLj[z];       // SLLj      [mM/ms]
	  dbuf_SLLsl[z] = 100.*Casl[z]*(37.4e-3*Vmyo/Vsl-buf_SLLsl[z])-1300.0e-3*buf_SLLsl[z];      // SLLsl     [mM/ms]
	  dbuf_SLHj[z] = 100.*Caj[z]*(1.65e-4*Vmyo/Vjunc-buf_SLHj[z])-30.0e-3*buf_SLHj[z];      // SLHj      [mM/ms]
	  dbuf_SLHsl[z] = 100.*Casl[z]*(13.4e-3*Vmyo/Vsl-buf_SLHsl[z])-30.0e-3*buf_SLHsl[z];     // SLHsl     [mM/ms]
	  J_CaB_junction = dbuf_SLLj[z]+dbuf_SLHj[z];
	  J_CaB_sl = dbuf_SLLsl[z]+dbuf_SLHsl[z];
	  ...
	  dNaj[z] = -I_Na_tot_junc*Cmem/(Vjunc*Frdy)+J_na_juncsl/Vjunc*(Nasl[z]-Naj[z])-dbuf_Naj[z];
	  dNasl[z] = -I_Na_tot_sl*Cmem/(Vsl*Frdy)+J_na_juncsl/Vsl*(Naj[z]-Nasl[z])+J_na_slmyo/Vsl*(Nai[z]-Nasl[z])-dbuf_Nasl[z];
	  dNai[z] = J_na_slmyo/Vmyo*(Nasl[z]-Nai[z]);             // [mM/msec] 
	  ...
	  dCaj[z] = -I_Ca_tot_junc*Cmem/(Vjunc*2.*Frdy)+J_ca_juncsl/Vjunc*(Casl[z]-Caj[z])-J_CaB_junction+(J_SRCarel)*Vsr/Vjunc+J_SRleak*Vmyo/Vjunc;  // Ca_j
	  dCasl[z] = -I_Ca_tot_sl*Cmem/(Vsl*2.*Frdy)+J_ca_juncsl/Vsl*(Caj[z]-Casl[z]) + J_ca_slmyo/Vsl*(Cai[z]-Casl[z])-J_CaB_sl;   // Ca_sl
	  dCai[z] = -J_serca*Vsr/Vmyo-J_CaB_cytosol +J_ca_slmyo/Vmyo*(Casl[z]-Cai[z]);
	  ...
	  RYRr[z] = RYRr[z]+dt*dRYRr[z];
	  RYRo[z] = RYRo[z]+dt*dRYRo[z];
	  RYRi[z] = RYRi[z]+dt*dRYRi[z];
	  buf_Naj[z] = buf_Naj[z]+dt*dbuf_Naj[z];
	  buf_Nasl[z] = buf_Nasl[z]+dt*dbuf_Nasl[z];
	  buf_TnC_low[z] = buf_TnC_low[z]+dt*dbuf_TnC_low[z];
	  buf_TnC_high[z] = buf_TnC_high[z]+dt*dbuf_TnC_high[z];
	  buf_Mg[z] = buf_Mg[z]+dt*dbuf_Mg[z];
	  buf_CaM[z] = buf_CaM[z]+dt*dbuf_CaM[z];
	  buf_myoCa[z] = buf_myoCa[z]+dt*dbuf_myoCa[z];
	  buf_myoMg[z] = buf_myoMg[z]+dt*dbuf_myoMg[z];
	  buf_SR[z] = buf_SR[z]+dt*dbuf_SR[z];
	  buf_SLLj[z] = buf_SLLj[z]+dt*dbuf_SLLj[z];
	  buf_SLLsl[z] = buf_SLLsl[z]+dt*dbuf_SLLsl[z];
	  buf_SLHj[z] = buf_SLHj[z]+dt*dbuf_SLHj[z];
	  buf_SLHsl[z] = buf_SLHsl[z]+dt*dbuf_SLHsl[z];
	  Naj[z] = Naj[z]+dt*dNaj[z];
	  Nasl[z] = Nasl[z]+dt*dNasl[z];
	  Nai[z] = Nai[z]+dt*dNai[z];
	  Caj[z] = Caj[z]+dt*dCaj[z];
	  Casl[z] = Casl[z]+dt*dCasl[z];
	  Cai[z] = Cai[z]+dt*dCai[z];
	...
	for (cc=0; cc<5; cc++) {
	  for (z=1; z<DIM-1; z++)  {
		Vn[z] = dt/(dx*dx)*D*(Vo[z+1]+Vo[z-1]-2.0*Vo[z]) + Vo[z];
	  }
	 ...
	 /* 
	  // APD
	  if (Vn[49]>=-70. && VP[49]<-70.) APDstart = t;
	  if (Vn[49]<=-70. && VP[49]>-70.) {
	    outputAPD = fopen("grandi_APD_cable_3c5_1Hz_k1_1000AP.dat","a");
	    fprintf(outputAPD,"%.2f\t%.2f\n",t,t-APDstart);
	    fclose(outputAPD);
	  }
		  
	  // Vdot_max
	  if (dV[49]/dt>dVmax) dVmax = dV[49]/dt;
	  if (Vn[49]<=0. && VP[49]>0.) {
	    outputVdmax = fopen("grandi_Vdmax_cable_3c5_1Hz_k1_1000AP.dat","a");
	  ...
	  // CV
	  if (Vn[19]>=0. && VP[19]<0.) CVstart = t;
	  if (Vn[79]>=0. && VP[79]<0.) {
	    outputCV = fopen("grandi_CV_cable_3c5_1Hz_k1_1000AP.dat","a");
